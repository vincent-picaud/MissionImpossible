...
- row by row using the =Jacobian_row()= function (uses reverse mode)
- column by column using the =Jacobian_column()= function (uses forward mode)
...
*Note:* in application we often encounter real functions. In that case
  ...
  represent the gradient of f denoted by ∇fₓ:
