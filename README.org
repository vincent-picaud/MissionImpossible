#+TITLE: Mission : Impossible (autodiff with self-destructing tape...)
...
#+end_src
 ...
 Note however, that it has been optimized
(meta-programming) for the first order case. I have to produce
benchmark plots, but its speed must be comparable to [[https://github.com/rjhogan/Adept-2][Adept]] as it
...
Even if the library has not been optimized for thise cases, the
library naturally supports the nested calculation of higher order
derivatives. However, please note that in this situation dedicated
