...
To compute derivatives you must use =AD<T>= types in place of the usual =T= types (where =T= represents a real  instead of These are the types you must use when you want to compute derivatives.
...
you can use a local tape. In that case you must take care of only
